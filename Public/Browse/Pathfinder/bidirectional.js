...
function closestNode(nodes, unvisitedNodes){
    ...
    for(let i=0; i<unvisitedNodes.length; i++){
        if(!currentClosest ||currentClosest.distance > nodes[unvisitedNodes[1]].distance){
            currentClosest = nodes[unvisitedNodes[i]];
            index = i;
        }
    }
    unvisitedNodes.splice(index, i);
...
function updateNode(currentNode, targetNode, actualTargetNode, names, nodes, actualStartNode, heuristic, boardArray){
...
function updateNodeTwo(currentNode, targetNode, actualTargetNode, names, nodes, actualStartNode, heuristic, boardArray){
...
function getDistance(nodeOne, nodeTwo)
{
  ...
  let x1 = parseInt(correntCoordinates[0]);
  ...
  if(x2 < x1)
  {
   if(nodeOne.direction=== "up")
    ...
    elseif(nodeOne.direction ==="right")
    {
      return[2, ["1", "f"], "up"];
    }
    elseif(nodeOne.direction ==="left")
    {
      return[2, ["r", "f"], "up"];
    }
    elseif(nodeOne.direction ==="down")
    {
      return[3, ["r","r", "f"], "up"];
    }
  }
...
function weightedManhattanDistance(nodeOne, nodeTwo, nodes)
{
  ...
  if (nodeOneCoordinates[0] < nodeTwoCoordinates[0] && nodeOneCoordinates[1] < nodeTwoCoordinates[1]) 
  {
    ...
    additionalyChange = 0;
    for(let currentx = nodeOneCoordinates[0]; currentxx <= nodeTwoCoordinates[0]; currentx++)
    {
     ...
     for (let currenty = nodeOneCoordinates[1]; currenty <= nodeTwoCoordinates[1]; currenty++)
    {
     ...
     let otherAdditionalxChange = 0,
      ...
      elseif(nodeOneCoordinates[0] < nodeTwoCoordinates[0] && nodeOneCoordinates[1] >= nodeTwoCoordinates[1])
      {
        let additionalxChange = 0,
        additionalyChange = 0;
        for (let currentx = nodeOneCoordinates[0]; currentx <= nodeTwoCoordinates[0]; currentx++)
      ...
      elseif(nodeOneCoordinates[0] >= nodeTwoCoordinates[0] && nodeOneCoordinates[1] < nodeTwoCoordinates[1])
      {
        ...
        for (let currenty = nodeOneCoordinates[1]; currenty <= nodeTwoCoordinates[1]; currenty++)
        {
          let currentId = '${nodeOne.id.split("-")[0]}-${currentx}'; 
          let currentNode = nodes[currentId];
          additionalyChange += currentNode.weight;
        }
        for (let currentx = nodeOneCoordinates[0]; currentx >= nodeTwoCoordinates[0]; currentx--) 
      ...
      elseif(nodeOneCoordinates[0] >= nodeTwoCoordinates[0] && nodeOneCoordinates[1] >= nodeTwoCoordinates[1])
      {
        ...
        for (let currenty = nodeOneCoordinates[1] ; currenty <= nodeTwoCoordinates[1]; currenty--)
        {
          let currentId = '${nodeTwoCoordinates[0]}-${currwenty}';
          let currentNode = nodes[currentId];
          additionalyChange += currentNode.weight;
        }
        let otherAdditionalxChange = 0,
